...
async function loadData() {
  if (!isLoggedIn()) {
    router.push(`/login?redirect=/products/${route.params.id}/edit`)
    return
  }
  if (!isAdmin()) {
...
async function submit() {
  ...
  try {
    ...
    success.value = '保存成功'
    setTimeout(() => router.push(`/products/${route.params.id}`), 800)
  } catch (err) {
    error.value = err.message
  } finally {
    saving.value = false
...
<template>
  <section class="edit-page">
    ...
    <div class="edit-card">
      ...
      <div v-else-if="error && !form.title" class="state-block error">
        <p>{{ error }}</p>
      </div>
